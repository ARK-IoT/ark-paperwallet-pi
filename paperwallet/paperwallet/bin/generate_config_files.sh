...
#
# Generate severall configuration files out of paperwallet.conf
...
generate_lighttpd_env() {
	...
	LIGHTTPD_ENV="setenv.add-environment = (
	   \"PYTHONPATH\"             => \"$PYTHONPATH:$PAPERWALLET/python_lib\",
...
if [ -z  $1 ] ; then
  echo "Usage is
      generate_config_files.sh /opt/paperwallet/conf/paperwallet.conf
   "
   exit 255
fi
...
. $NODE_CONFIG
. $PAPERWALLET_FOLDER/lib/node_name_generation.sh

IPV6="#"

set_pathnames  $PAPERWALLET_FOLDER
...
generate_lighttpd_env "$GLOBAL_DEST" $PAPERWALLET_PYTHONPATH $PAPERWALLET_FOLDER $GEN_DISKFILE
...
### PaperWallet Scripts generates its own config in  /opt/paperwallet/conf/avahi
###   but, the daemon works per default only on /etc/avahi
### If you want to enable avahi, then you have to link /etc/avahi to /opt/paperwallet/conf/avahi
### On OpenWRT this should happen, if avahi is available before installing the paperwallet
###  automtically.
