...
CONFIG_PATH="/opt/paperwallet/conf/hostapd.conf"
PAPERWALLET_CONFIG_PATH="/opt/paperwallet/conf/paperwallet.conf"
...
paperwallet_interface=$( grep -e '^INTERFACE' "${PAPERWALLET_CONFIG_PATH}" | \
                sed -e 's|INTERFACE=||' -e 's|"||g' )
dnsmasq_interface=$( grep -e '^DNSMASQ_INTERFACE' "${PAPERWALLET_CONFIG_PATH}" | \
                sed -e 's|DNSMASQ_INTERFACE=||' -e 's|"||g' )

sed -i -e "s|interface=$hostap_interface|interface=$WIFI_DEVICE|" "${CONFIG_PATH}"

#Only change paperwallet interface if it is a wifi interface
if echo "$paperwallet_interface" | grep -q "wlan" ; then
    sed -i -e "s|INTERFACE=\"$paperwallet_interface\"|INTERFACE=\"$WIFI_DEVICE\"|" \
         "${PAPERWALLET_CONFIG_PATH}"
fi
...
if [ -f /boot/wpa_supplicant.conf ]; then
  /opt/paperwallet/rpi/bin/run_client.sh
  exit 1
  # Exit =! 0 will result in not starting paperwallet service
fi
